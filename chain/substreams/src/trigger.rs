    ...
    T: RuntimeHostBuilder<Chain>,
{
    async fn process_trigger(
        ...
        _hosts: &[Arc<T::Host>],
        block: &Vec<Arc<Block>>,
        _trigger: &Vec<Vec<TriggerData>>,
        mut state: Vec<BlockState<Chain>>,
        proof_of_indexing: &SharedProofOfIndexing,
        causality_region: &str,
        _debug_fork: &Option<Arc<dyn SubgraphFork>>,
        _subgraph_metrics: &Arc<graph::prelude::SubgraphInstanceMetrics>,
    ) -> Result<Vec<BlockState<Chain>>, MappingError> {
        for i in 0..block.len(){
            for entity_change in block[i].entities_changes.entity_changes.iter() {
                match entity_change.operation() {
                    ...
                    Operation::Create | Operation::Update => {
                        ...
                        state[i].entity_cache.set(key, Entity::from(data))?;
                    }
                    Operation::Delete => {
                        let entity_type: &str = &entity_change.entity;
                        let entity_id: String = String::from_utf8(entity_change.id.clone())
                        ...
                        state[i].entity_cache.remove(key);
