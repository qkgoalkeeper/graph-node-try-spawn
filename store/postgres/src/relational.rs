...
impl SqlName {
    ...
    pub fn query_name(namespace: &Namespace, name: &SqlName) -> Self {
        SqlName(format!("{}.{}", namespace, name.as_str()))
    }
}
...
impl Layout {
    ...
    fn make_poi_table(catalog: &Catalog, position: usize) -> Table {
        ...
        Table {
            ...
            immutable: false,
            query_name: SqlName::query_name(&catalog.site.namespace, &table),
        }
    }
    ...
    pub fn insert<'a>(
        ...
        /*for (key,entity) in entities{
            ...
            let op=entity.get("operation");
            let de=entity.get("delta");
            match (id,op,de){
                (Some(Value::Bytes(b)),Some(Value::Int(o)),Some(Value::BigInt(v))) => {
                    println!("---qualified_name: {:?}, name: {:?}",table.qualified_name.as_str(),table.name.as_str());
                    //let ids=b.to_string().split_at(1).1;
                    let c=b.to_string();
                    let len=c.len();
                    let ids=&c[1..len];
                    let x=&ids.to_string();
                    println!("ids:{:?}",ids);
                    let y=format!("'\\{}'",x);
                    let querysql="SELECT delta From ".to_string()+&table.query_name.as_str().to_string()+&" where id=".to_string()+&y.to_string();
                    println!("---query_sql={:?}---",querysql.clone());
                    let result=conn.batch_execute(querysql.as_str());
                    println!("---query_result={:?}---",result);
        ...
        println!("insert_number:{:?}",count.clone());
        Ok(count)
    }
    ...
    pub fn update<'a>(
       ...
       /* for (key,entity) in entities{
        ...
        println!("update_number:{:?}",count.clone());
        Ok(count)
    }
...
impl Table {
    fn new(
        ...
        let query_name=SqlName::query_name(&catalog.site.namespace, &table_name);
        let table = Table {
