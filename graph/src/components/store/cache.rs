...
impl EntityCache {
    ...
    pub fn as_modifications(mut self) -> Result<ModificationsAndCache, s::QueryExecutionError> {
        ...
        //从store里面把那些current里面没有的entites取出来
        ...
        for (key, update) in self.updates { //这边逻辑需要修改，最方便的就是只要是updates里面有，就要记录到mods里面,然后把改动0的去掉
            ...
            let modification = match (current, update) {
                ...
                (Some(current), EntityOp::Update(updates)) => {
                    ...
                    //updates.combine(current);
                    data.merge_remove_null_fields(updates);
                    data.combine(current.clone());
                    self.current.insert(key.clone(), Some(data.clone()));
                    if current != data {
            ...
            if let Some(modification) = modification {
                // println!("--Modify: {:?}",modification.clone());
                mods.push(modification);
                /*let x=modification.clone();
                let a=x.entity().unwrap().get("delta");
                match a{
                    Some(Value::BigInt(v)) => {
                        let c=v.clone();
                        let cc=v.clone();
